...
- Added `.handoffignore`, `.github/ISSUE_TEMPLATE`, `scripts/export-test-drop.ps1`, `scripts/publish-delivery-repo.ps1`, and `docs/GITHUB_HANDOFF.md`. `export-test-drop.ps1 -Zip` regenerates a temporary sanitized drop under `D:\work2.0\.tmp\SICUN-Portal_test` and a zip under `D:\work2.0\.tmp\SICUN-Portal_handoff\releases`, excluding internal files and build/dependency outputs. Hermes should feed bug reports and suggestions back through Issues in `SICUN-Portal-delivery`.
...
- Fixed Hermes workflow generation in `scripts/export-test-drop.ps1` to use a single-quoted PowerShell here-string so markdown backticks such as `test-drop/*` and `bug-report-*` are preserved literally.
